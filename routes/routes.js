...
router.get("/", requireAuth, function (req, res) {
        ...
        /*
                if (!req.session.username && !Group.mysqlIsImported) {
                        loadGroups();
                        res.render("login01", { groups: Group.groups });
                }
 ...
 * Group login post.
 */
router.post('/login01.ejs', function (req, res) {
        let errorMessage = "";
...
* User login post.
*/
router.post('/login02.ejs', function (req, res) {
...
router.get("/login01.ejs", function (req, res) {
        if (!req.session.username && !Group.mysqlIsImported) {
                loadGroups();
                res.render("login01", { groups: Group.groups });
        }
...
router.get("/login02.ejs", function (req, res) {
        if (!req.session.username && !Group.mysqlIsImported && Group.group == "") {
                ...
                res.render("login02", { group: Group.group });
        }
        else {
                return res.redirect("/hub.ejs");
        }
...
router.get("/hub.ejs", function (req, res) {
        if (req.session.username) {
                // LARA 02.08.2016
                usbString = usbCheckFree();
                res.render("hub", { username: req.session.username, group: Group.group, color: req.session.usercolor, usbString: usbString });
                // LARA end
        }
        else {
                return res.redirect("/login01.ejs");
