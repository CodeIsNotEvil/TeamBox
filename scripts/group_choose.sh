#!/bin/bash
#
# /home/ubuntu/app/scripts/group_choose.sh
#
...
# This will call the script group_create.sh.
...
then
	...
	then
		if [ ! "$group" = "" ]
		then
			sudo bash group_create.sh "$group"
			exit_status="$?"
		fi
	else
		exit_status=1
