...
#!/bin/bash
#
# /home/pi/scripts/server_start.sh
#
#
...
if [ -z "$app" ]
then
	node /var/www/html/app/server.js &
	printf "\nStarted app"
else
...
if [ -z "$etherpad" ]
then
	sudo -H -u pi bash -c /home/pi/etherpad-lite/bin/run.sh &
	printf "\nStarted etherpad"
else
...
if [ -z "$ethercalc" ]
then
	# first change directory (to save dump in pi's home)
	cd /home/pi
	sudo -H -u pi bash -c ethercalc &
	printf "\nStarted ethercalc"
else
